  ...
  <div class="flex min-h-screen bg-gray-100">
    ...
    <div class="container w-full p-6 flex-1 ml-64">
      ...
      <div class="flex justify-between items-center">
        ...
        <!-- Add Item Form -->
        <NuxtLink to="/form-frame" class="mb-2 py-2 px-6 rounded-full transition ease-in-out delay-100 bg-green-400 text-sm hover:scale-[1.05] hover:bg-green-500 hover:text-white duration-300">+ Add Item</NuxtLink>
      </div>
      ...
      <div class="table-container max-h-[500px] overflow-y-auto rounded-lg border border-gray-200">
        <table class="min-w-full bg-white border border-gray-100 rounded-lg shadow-md">
          ...
          <tbody>
            <tr v-for="(item, index) in data.frames" :key="index" class="border-b hover:bg-gray-50 text-sm">
              <td class="py-3 px-4">{{ item.ID }}</td>
              <td class="py-3 px-4">
                <img :src="item.Url" class="w-12" alt="" />
              </td>
...
const fetchData = async () => {
  ...
  const url = `https://services.snaplab.id/api/v1/frame/?page=${currentPage.value}`;

<template>
  <div class="flex min-h-screen bg-gray-100">
    ...
    <div class="container w-full p-6 flex-1 ml-64">
      <h1 class="text-2xl font-semibold mb-1">Promo Table</h1>
      ...
      <div class="flex justify-between items-center">
        ...
        <!-- Add Item Form -->
        <NuxtLink to="/form-promo" class="mb-2 py-2 px-6 rounded-full transition ease-in-out delay-100 bg-green-400 text-sm hover:scale-[1.05] hover:bg-green-500 hover:text-white duration-300">+ Add Item</NuxtLink>
      </div>
        ...
        <table class="min-w-full bg-white border border-gray-100 rounded-lg shadow-md">
          <thead class="bg-gray-300">
            <tr>
              ...
              <th class="py-3 px-4 text-left text-sm font-semibold text-gray-700">KODE PROMO</th>
              <th class="py-3 px-4 text-left text-sm font-semibold text-gray-700">DISKON</th>
              <th class="py-3 px-4 text-left text-sm font-semibold text-gray-700">COUNTER</th>
              <th class="py-3 px-4 text-left text-sm font-semibold text-gray-700">TERBATAS</th>
              <th class="py-3 px-4 text-left text-sm font-semibold text-gray-700">TERSEDIA</th>
              <th class="py-3 px-4 text-left text-sm font-semibold text-gray-700">EXPIRY DATE</th>
          ...
          <tbody>
            <tr v-for="(item, index) in data.promos" :key="index" class="border-b hover:bg-gray-50 text-sm">
              <td class="py-3 px-4">{{ item.ID }}</td>
              <td class="py-3 px-4">{{ item.Code }}</td>
              <td class="py-3 px-4">{{ item.Discount }}</td>
              <td class="py-3 px-4">{{ item.Counter }}</td>
              <td class="py-3 px-4">{{ item.Limited ? "Iya" : "Tidak" }}</td>
              <td class="py-3 px-4">{{ item.Available ? "Iya" : "Tidak" }}</td>
              <td class="py-3 px-4">{{ formatDate(item.ExpiryDate) }}</td>
...
const fetchData = async () => {
  ...
  const url = `https://services.snaplab.id/api/v1/promo/?page=${currentPage.value}`;
...
const saveItem = async () => {
  const token = localStorage.getItem("authToken");
  const url = `https://services.snaplab.id/api/v1/promo/${currentItem.value.ID}`;
